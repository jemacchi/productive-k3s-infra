...
if printf '%s\n' "${default_scs}" | grep -q '^local-path|true$'; then
  ...
  mp_exec "${SERVER_NAME}" "sudo k3s kubectl patch storageclass local-path -p '{\"metadata\":{\"annotations\":{\"storageclass.kubernetes.io/is-default-class\":\"false\"}}}'"
fi

if ! printf '%s\n' "${default_scs}" | grep -q '^longhorn|true$'; then
  log "Marking longhorn as default StorageClass"
  mp_exec "${SERVER_NAME}" "sudo k3s kubectl patch storageclass longhorn -p '{\"metadata\":{\"annotations\":{\"storageclass.kubernetes.io/is-default-class\":\"true\"}}}'"
fi
